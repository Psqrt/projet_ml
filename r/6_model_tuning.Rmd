    ...
    for (feature_fraction_param in c(0.7, 0.8, 0.9)) {
      for (bagging_fraction_param in c(0.6, 0.7, 0.8)) {
        ...
        row_param = data.frame(
          ...
          "validation" = up_lgb_cv[["record_evals"]][["valid"]][["Norm-gini"]][["eval"]] %>% unlist %>% max
        )
...
```{r}
# save(up_lgb_tune_df, file = "./../output/up_lgb_tune_df.RData")
load("./../output/up_lgb_tune_df.RData")
...
up_lgb_info = data.frame("Method" = "lgb",
                         ...
                         "Validation" = ngini_valid,
                         "Test" = up_lgb_test_ngini,
                         stringsAsFactors = F)

# save(up_lgb_info, file = "./../output/up_lgb_info.RData")
```
...
```{r}
# save(none_lgb_tune_df, file = "./../output/none_lgb_tune_df.RData")
load("./../output/none_lgb_tune_df.RData")
...
# save(none_lgb_info, file = "./../output/none_lgb_info.RData")
```

## Stacking

Après avoir comparé les performances de modèle (Rmd n°7), on tente un stacking de deux modèles : LGB et XGB sans resampling.
...
### Stacking sans fuite d'information
...
```{r}
train_preds = predict(glm_stacking_model$finalModel, df_stacking_train_X, type = "response")
train_gini = normalizedGini(train_Y_num, train_preds)
valid_gini = max(glm_stacking_model$results$NormalizedGini)
c("Train" = train_gini,
  "Valid" = valid_gini,
  "Test" = NA)
...
```{r}
# save(glm_stacking_model_info, file = "./../output/glm_stacking_model_info.RData")
```


Le score de validation est le meilleur par rapport aux autres modèles.

### Stacking avec alpha (combinaison)
...
df_results_valid_combinaison %>% 
  ...
  annotate(geom = "text", x = 0.08, y = 0.2776, label = "100% LGB") +
  annotate(geom = "text", x = 0.97, y = 0.277, label = "100% XGB") +
  geom_point()
```
...
gini_train = normalizedGini(train_Y_num, df_train_combi$combinaison)
gini_test = normalizedGini(test_Y_num, df_test_combi$combinaison)
...
```{r}
# save(combinaison_model_info, file = "./../output/combinaison_model_info.RData")
```
