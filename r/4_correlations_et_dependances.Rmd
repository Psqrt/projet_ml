...
On note que 6 variables ne passent pas l'ANOVA. Ce sont essentiellement des variables type `calc` : `ps_calc_11`, `ps_calc_14`, `ps_calc_02`, `ps_calc_10`, `ps_car_11` et `ps_calc_03`. Ces variables sont écartées pour la suite.

```{r}
var_poubelle = df_recap_anova[df_recap_anova$pvalue > 0.05, "variable"]
```
...
Au final, on retient 29 variables pour la construction des modèles.
...
save(train, test, type_col,
     file = "./../data/bases_29features.RData")
```
